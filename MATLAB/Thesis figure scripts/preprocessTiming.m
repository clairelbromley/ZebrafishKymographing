function output = preprocessTiming(varargin)
...
userOptions.outputFolder = 'C:\Users\Doug\Google Drive\Preprocessing timing test';
...
%                     end
                ...
                if ~userOptions.scatterComparisonOnly
                    ...
                    if (userOptions.saveCutPositioningFigs)
                        ...
                        %% Pre-process images in stack - looping through different approaches
                        oldstack = stack;
                        userOptions.preProcess = false;
                        unprocessedTimer = tic;
                            [stack, curr_metadata] = kymographPreprocessing(oldstack, curr_metadata, userOptions);
                        unprocessedTime = toc(unprocessedTimer);
                        userOptions.preProcess = true;
                        userOptions.erosionDilation = false;
                        mfOnlyTimer = tic;
                            [stack, curr_metadata] = kymographPreprocessing(oldstack, curr_metadata, userOptions);
                        mfOnlyTime = toc(mfOnlyTimer);
                        userOptions.erosionDilation = true;
                        fullPPTimer = tic;
                            [stack, curr_metadata] = kymographPreprocessing(oldstack, curr_metadata, userOptions);
                        fullPPTime = toc(fullPPTimer );
...
close all;
fullTime = toc(fullProcessTimer);
disp(sprintf('The time for the preprocessing function WITHOUT MEDIAN FILTER OR NOISE REMOVAL was %f seconds', unprocessedTime));
disp(sprintf('The time for the preprocessing function WITH MEDIAN FILTER but WITHOUT NOISE REMOVAL was %f seconds', mfOnlyTime));
disp(sprintf('The time for the preprocessing function WITH MEDIAN FILTER AND NOISE REMOVAL was %f seconds', fullPPTime));
disp(sprintf('The *entire* process, including *all three* preprocessing types, took %f seconds', fullTime)); 
disp(sprintf('The time taken for all the steps EXLUDING ANY PREPROCESSSING was %f seconds', (fullTime - unprocessedTime - mfOnlyTime - fullPPTime)));
end
