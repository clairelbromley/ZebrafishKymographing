function midline_definition = calc_midline_definition(data, edge)
    ...
    %         [y, I] = sort(y);
        ...
        if all(edge.edgeValidity(3, :))
            ...
            midline_definition = generate_stats(midline_definition, 'midline_def', stats, {'AllRh'}, double(midline_definition_array));
...
%             midline_definition.AllRh.max_midline_def = max(midline_definition_array);
        ...
        for rh = rhs
            if edge.edgeValidity(3, rh-min(rhs)+1)
                ...
                midline_definition = generate_stats(midline_definition, 'midline_def', stats, {rh}, double(temp_m));
            end
